...
Can also use rename function to rename a column without losing the other
columns
...
Select helpers - functions that help you select a column based on a
criteria
...
Removes or keeps rows that you’re interested in retaining for select
analyses Need to give it a variable and a condition so it knows what to
do
...
negate things –\> gd\_of\_birth when it is NOT equal to 20
...
Modify existing variable or create a new variable Modify group variable
to making everything lower case
...
Put things in order (ascending) Sort by 2 things - first by the first
variable and then again in order within those levels
